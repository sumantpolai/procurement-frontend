...
export default function PRListPage() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // The search API returns {data: [...], page, limit, total} format
      if (response.data && Array.isArray(response.data)) {
        setPrs(response.data);
        setTotal(response.total || response.data.length);
      } else {
        // Fallback: if it returns a single PR object
        setPrs([response]);
        setTotal(1);
      }
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Purchase Requests ({total})</CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="text-center py-8 text-gray-500">Loading...</div>
          ) : prs.length === 0 ? (
            <div className="text-center py-8 text-gray-500">No purchase requests found</div>
          ) : (
            <>
              <Table>
                <TableHeader>
